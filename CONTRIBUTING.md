...
- Markdown only. No HTML. No images that aren't already next to the source-of-truth doc.
...
| 2026-05-10 | source-of-truth pass | actual SDK source files (the RN, KMP, and iOS-sample SDK sources) | Cross-checked the cheatsheets against the real Kotlin/Swift/TS source. Three material corrections: (1) `AmplyConfig` on iOS / Android / KMP requires `apiKeySecret` and `defaultConfig` — every native init example was missing both, fixed. (2) RN cheatsheet was missing `getRecentEvents`, `getDataSetSnapshot`, `removeAllListeners`, `systemEvents.addListener` aliases — added; `getCustomProperty` return type clarified to `string \| number \| boolean \| null`. (3) Custom Property limits documented (RN: keys ≤ 32 chars + values ≤ 255 chars; native: keys ≤ 255 chars). Also: KMP iOS init clarified — Kotlin/Native Swift bridge maps `initialize` → `doInitialize`. **Added Phase 0 (Context7 MCP)** so the skill prefers live `docs.amply.tools` over the bundled cheatsheets when available, and offers to install Context7 when missing — see `references/context7-mcp.md`. |
| 2026-05-18 | mode-switch + phase-1b + ensure_app integration | `SKILL.md`, `references/amply-mcp.md` | Closes the "skill mints duplicate keys on every re-run" regression: introduced `amply_ensure_app` (idempotent project+app+key resolution, explicit `mintNewKey`, cross-project conflict guard) and `amply_find_application` (pure-read discovery). `amply_bootstrap_for_app` deprecated, kept as thin wrapper for one release. Skill gains an `autopilot` vs `interactive` mode resolved once at the top of the run (message arg / env / one-time prompt) and consulted by every multi-option phase decision. New Phase 1b "Amply app resolution" sub-step calls `amply_ensure_app` once and produces the `envBlock` Phase 5 consumes — Phase 5 no longer discovers keys. Codex review #1 reshaped the four-workstream plan; codex review #2 caught admin-environment handling, a route-name collision (resolved by namespacing the MCP's routes), unauth status (400 not 401), and cross-project edge cases — all folded before code. Field-tested against an existing RN bare project: re-running the skill no longer mints new keys silently. |
| 2026-05-12 | Amply MCP shipped (companion package) | Amply MCP (separate npm package `@amplytools/amply-mcp`) | Implemented a local stdio MCP server backed by the Amply admin GraphQL API. 12 tools (signup / login / logout / whoami / list+create projects / list+get+create applications / create api key / bootstrap_for_app). One-shot `amply_bootstrap_for_app` returns a ready-to-paste `.env.local` block including `appId` (bundleId) + `apiKeyPublic` + `apiKeySecret`. Skill changes: `SKILL.md` Phase 0 now checks both Context7 AND Amply MCP; Phase 5 mentions `amply_bootstrap_for_app` as the canonical key source when the MCP is connected; new `references/amply-mcp.md` documents tools, error codes, autonomous-mode policy, and the one-shot flow. Codex review #1 of the MCP plan caught 25 contract / security / SDK issues; all folded before the v1 build. Codex review #2 of the actual code pending overnight. Public release path coordinated alongside the skill's. |
...
| 2026-05-28 | MCP 0.3.0 coordinated docs refresh | `SKILL.md`, `references/amply-mcp.md` | Dropped references to `amply_bootstrap_for_app` (removed from MCP 0.3.0; the skill's Phase 1b already used `amply_ensure_app`). Added the three new MCP authoring tools to the tool table so agents can discover the campaign-authoring vocabulary (`amply_create_campaign`, `amply_update_campaign`, `amply_describe_targeting`). Skill behavior unchanged — runtime path is the same. |

| 2026-05-31 | plugin distribution + curl pre-approval | `README.md`, `SKILL.md`, `.claude-plugin/`, `plugins/` | Two distribution/UX items. (1) Made the repo installable as a Claude Code plugin via a single-plugin marketplace (`marketplace.json` + `plugins/amply-integration/` with the skill symlinked from the canonical root files — single source of truth, no copy/drift). Empirically verified the `skills` CLI still finds exactly one skill with the plugin tree present, in both default and full-depth scans, so the primary `npx skills add` path is unaffected. README gained a 4-CLI install table (Claude Code / Codex / Copilot / Gemini) + the plugin one-liner. (2) New Phase 0.7 pre-approves the skill's `curl`s in `.claude/settings.local.json` (local, git-ignored) so Phase 9 telemetry and any docs fallback don't trigger per-call permission prompts; idempotent, telemetry rule gated on `feedbackConsent`, no-op on non-Claude-Code hosts. Allow-rule form uses the trailing ` *` prefix-glob required by Claude Code's matcher. A second-opinion review preferred a fully separate marketplace repo to eliminate any scan-collision risk; overridden after the no-collision behaviour was verified directly against the `skills` CLI, in favour of the single-source-of-truth symlink layout. |

| 2026-06-03 | marketplace → dedicated catalog repo | `plugins/`, `scripts/`, `CHANGELOG.md`, `README.md` | Reworked the Claude Code plugin distribution after the single-repo form (`marketplace add amply-tools/sdk-skill`) was found to read like "install the skill" rather than "add a catalog". Moved the marketplace to a dedicated catalog repo `amply-tools/claude-plugins` (marketplace name `amply`) that references this repo's plugin via a `git-subdir` source; new commands are `marketplace add amply-tools/claude-plugins` + `install amply-integration@amply`. A `git-subdir` sparse clone can't follow symlinks into the repo root, so the plugin subtree was converted from symlinks to real copies of `SKILL.md` + `references/`, regenerated by `scripts/sync-plugin.sh` with a `--check` drift gate at release. Verified `npx skills add` still finds exactly one skill (default and full-depth) with the real copies present, so the primary install is unaffected. Naming + storage approach each second-opinion-reviewed; `sdk@amply` was considered but `amply-integration@amply` chosen so the plugin name matches the skill name. |
| 2026-07-18 | event-history targeting + MCP catalog reconcile (v0.10.0) | `SKILL.md`, `references/who-when-what-audit.md`, `custom-properties.md`, `amply-mcp.md`, cheatsheets, `README.md` | Amply campaigns gained direct event-history audience targeting (Amply SDK 0.6.1+): event counts (`how many times`) with six comparison ops plus `has happened` (ever) / `has never happened`, `first occurrence` / `last occurrence` dates (relative incl. an "…or never" variant, and absolute whole-UTC-day bounds), event property filters on both, up to 20 conditions per campaign, evaluated on-device/offline with history starting at install. The skill previously taught the opposite ("event counts are not built-in — maintain `*_count` counters"), so every spot carrying that claim was rewritten in one pass: Who-model table gains a third source row, counter properties reframed as the fallback (older fleets + derived state), Phase 3 instruction, property-writes/event-naming/audit-template ripples, and a stronger bridge pitch (a forwarded event is now an audience condition, not just a trigger). Separately reconciled the MCP tool catalog against the shipped `@amplytools/amply-mcp` tool list — four campaign tools (`list`/`get`/`set_state`/`create_from_template`) were live and used by Phase 8 but missing from the reference table — and bumped all cheatsheet install pins to 0.6.1 with a version-floor callout, since a device below 0.6.1 silently never matches event-condition campaigns. README distribution section refreshed to the catalog-repo + real-copies model. |
| 2026-07-19 | Expo config-plugin guidance fix (v0.10.1) | `references/sdk-cheatsheet-rn.md`, `SKILL.md` | An integrator following the skill added the Amply entry to `app.json` → `expo.plugins` and was then told by the SDK itself that the plugin is no longer required. Root cause: guidance drift — the plugin has been a no-op since SDK 0.2.12 (autolinking registers the native module in every flavour, including during `prebuild`), but the cheatsheet and the Flavour C/D instructions still mandated the entry. Fixed in one pass: cheatsheet paragraph + flavour table, Flavour B/C/D verification bullets, Phase 1 flavour note, and the Common Mistakes row (now: no entry needed in any flavour; existing entries harmless — remove on next `app.json` touch; `prebuild` itself stays mandatory for Flavour C). |
