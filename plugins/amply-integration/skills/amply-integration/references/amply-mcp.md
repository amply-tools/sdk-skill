...
| `amply_create_campaign` | Create a campaign from a full definition — event property filters via `params`, every-N `repeat`, device / customProperty / event-history targeting (event-count and first/last-occurrence date conditions with per-event property filters; relative windows in minutes, hours or days via `relativeUnit` — default days; up to 20 event conditions per campaign, matched only by apps on Amply SDK 0.6.1+). Always created in Draft state. |
